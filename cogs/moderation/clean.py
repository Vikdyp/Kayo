...
class Clean(commands.Cog):
    ...
    async def clean_execute(
        ...
        message_id: Optional[str] = None,  # Changement en str
        ...
        if action.value == "all":
            ...
            if not await self.ask_confirmation(interaction, f"Confirmez-vous la suppression de tous les messages à partir du message `{msg_id_int}` dans {channel.mention} ?"):
                return await interaction.followup.send("Action annulée.", ephemeral=True)
